...
def load_utterances(filepath, clean=True):
    #try:
    data, speakers = preprocess_utterance(filepath)
    ...
    for row in data:
        ...
        # speakerが表記されているときだけ更新
        if row.startswith("*"):
            speaker = re.findall(r"\*(.+?):", row)[0]
            speaker = 0 if speaker == speakers[0] else 1
...
def cut_audio(audio_path, out_path, start, end):
    data, sr = sf.read(audio_path)
    sf.write(out_path, data[int(start*sr) : int(end*sr)], sr)
