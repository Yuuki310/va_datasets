...
if __name__ == "__main__":
    ...
    df = prepare_df(data_dir, meta_path, participant_path)
    df.to_csv(os.path.join(repo_root(), "datasets_turntaking", "datasets", dataset, "files/sessions.csv"), index=False)
    data = []
    for _, row in df.iterrows():
        transcript_path = os.path.join(data_dir, row["rec"], row["session"], row["talk"] + "-transUnit.csv")
        ...
        first_audio_path = os.path.join(data_dir, row["rec"], row["session"], row["talk"] + "_" + row["first_speaker_id"] + ".wav")
        second_audio_path = os.path.join(data_dir, row["rec"], row["session"], row["talk"] + "_" + row["second_speaker_id"] + ".wav")
        audio_path = os.path.join(audio_dir, row["talk"] + ".wav")
        ...
        l_audio, sample_rate = sf.read(first_audio_path)
        r_audio, sample_rate = sf.read(second_audio_path)
