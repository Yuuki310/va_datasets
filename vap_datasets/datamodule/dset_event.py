    ...
    for name in ["overlap_shift"]:
        ...
        for single_event in all_events:
            ...
            data.append(
                {
                    "ipu_end": frame_to_time(start),
                    "tfo": frame_to_time(start-end),
                    "speaker": other_speaker,
                    "label": name,
                }
            )
    for name in ["notime_shift"]:
        all_events = ee[name][0]  # no batch dim
        for single_event in all_events:
            start, end, next_speaker = single_event
            other_speaker = 0 if next_speaker == 1 else 1
...
if __name__ == "__main__":
    ...
    parser.add_argument("--post_cond_time", type=float, default=1.0)
    parser.add_argument("--min_silence_time", type=float, default=0)
